...
FileOperation.open = function() {
	...
	dialog.showOpenDialog(electron_renderer_Remote.getCurrentWindow(),{ properties : ["openFile","multiSelections"], filters : [{ name : "Visual Log", extensions : ["vilog"]}]},FileOperation.openFiles);
};
FileOperation.openFiles = function(filePaths) {
	var _g = 0;
	while(_g < filePaths.length) {
		var path = filePaths[_g];
		++_g;
		var path1 = [js_node_Path.resolve(path)];
		var existing = Main.goldenLayout.root.getItemsByFilter((function(path) {
			return function(item) {
				var state = item.config.componentState;
				if(state == null) {
					return false;
				}
				return state.path == path[0];
			};
		})(path1));
		if(0 < existing.length) {
			var target = existing[0];
			console.log("src/renderer/FileOperation.hx:42:",target);
			if(target.parent != null && target.parent.isStack) {
				target.parent.setActiveContentItem(target);
			}
		} else {
			console.log("src/renderer/FileOperation.hx:50:",path1[0]);
			...
			stack.addChild(Main.createFileContent(path1[0]));
		}
	}
};
...
FileOperation.findStackFromTop = function(item) {
	...
	console.log("src/renderer/FileOperation.hx:73:",item.isComponent);
	if(item.isComponent) {
...
Main.createFileContent = function(path) {
	return { type : "component", componentName : "file", componentState : { path : js_node_Path.resolve(path)}, id : "file_" + Std.random(134217727), title : js_node_Path.basename(path)};
};
